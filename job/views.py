...
def create_job(request):
    ...
    if userType == "Company":
        ...
        return render(
            request,
            "create_job.html",
            {"user": userJSON, "userType": userType, "company": company},
        )
    else:
    ...
    if request.method == "PATCH":
        ...
        try:
            ...
            if bodyJson["status"] == "ACCEPTED":
                ...
                payload[
                    "body"
                ] = f"Votre candidature pour '${job_dating.job.name}' a été accepté 😁!"
            elif bodyJson["status"] == "REJECTED":
                job_dating.status = "RE"
                payload["head"] = "Candidature Rejeté !"
                payload[
                    "body"
                ] = f"Votre candidature pour '${job_dating.job.name}' a été rejeté 😖!"
